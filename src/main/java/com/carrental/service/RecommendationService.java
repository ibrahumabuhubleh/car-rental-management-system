...
public class RecommendationService {
    ...
    private double calculateScore(Car car, int durationDays, String weather) {
        ...
        if (car.getCategory().equalsIgnoreCase("Economy")) score += 10;
        if (durationDays >= 7 && car.getFuelType().equalsIgnoreCase("Hybrid")) score += 20;
        if (weather != null && weather.toLowerCase().contains("rain") && car.getCategory().equalsIgnoreCase("SUV")) score += 25;
        if (car.getTransmission().equalsIgnoreCase("Automatic")) score += 5;
